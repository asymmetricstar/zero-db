...
interface NetworkState {
  nextSequence: number;
  activeLocks: { [resource: string]: LockInfo };
}

export class DataManager {
  private rootPath: string;
  private spawnPool: Map<string, Spawn>;
  private cacheManager: CacheManager;
  private autoIncrementCounters: Map<string, number> = new Map();
  private operationLocks: Map<string, Promise<any>> = new Map();
  private isNetwork: boolean;
  private readonly networkLockPath: string;
  private engineId: string;

  constructor(rootPath: string, cacheManager: CacheManager, isNetwork: boolean = false) {
  ...
  private getAutoIncrementKey(dbName: string, tableName: string): string {
    return `${dbName}:${tableName}`;
  }

  private getLockKey(dbName: string, tableName: string): string {
    return `${dbName}:${tableName}`;
  }
  ...
  private async readNetworkState(): Promise<NetworkState> {
    ...
    try {
      ...
      if (!content.trim()) return { nextSequence: 1, activeLocks: {} };
  ...
  private async writeNetworkState(state: NetworkState): Promise<boolean> {
  ...
  private async acquireDistributedLock(dbName: string, tableName: string): Promise<number> {
    ...
    EventManager.error(`Distributed lock timeout for ${resource}`);
    return -1;
  }

  private async releaseDistributedLock(dbName: string, tableName: string, sequence: number): Promise<void> {
    const resource = `${dbName}:${tableName}`;
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
      ...
      if (lock && lock.engineId === this.engineId && lock.sequence === sequence) {
        delete state.activeLocks[resource];
        if (await this.writeNetworkState(state)) return;
      } else {
        return;
      }
  ...
  private async withAtomicLock<T>(dbName: string, tableName: string, fn: () => Promise<T>): Promise<T> {
  ...
  private async syncAutoIncrementFromDisk(dbName: string, tableName: string, fileName: string): Promise<number> {
  ...
  private reserveAutoIncrement(dbName: string, tableName: string): number {
    const key = this.getAutoIncrementKey(dbName, tableName);
  ...
  private getSpawn(dbName: string, tableName: string, fileName: string): Spawn {
    const key = `${dbName}:${tableName}:${fileName}`;
    if (!this.spawnPool.has(key)) {
      const spawn = Spawn.create(this.rootPath, dbName, tableName, '', fileName);
  ...
  validateAndCastData(
  ...
  ): { valid: boolean; errors: string[]; castedData: Map<string, string> } {
    const errors: string[] = [];
    const castedData = new Map<string, string>();
    ...
    if (isUpdate) {
        for (const [fieldName, value] of data) {
            ...
            if (casted !== null) castedData.set(fieldName, casted);
        }
    } else {
        for (const [fieldName, schema] of schemas) {
            ...
            if (schema.type === 'timestamp') {
                if (value) castedData.set(fieldName, value);
                continue;
            }
  ...
  private castValue(fieldName: string, value: string, schema: FieldSchema, errors: string[]): string | null {
  ...
  async insertRecord(
    ...
    schemas?: Map<string, FieldSchema>
  ): Promise<{ success: boolean; lineNumber: number; errors: string[] }> {
  ...
  private async getNextAutoIncrement(dbName: string, tableName: string, fieldName: string, fieldFileNames: Map<string, string>): Promise<number> {
    const fileName = fieldFileNames.get(fieldName);
  ...
  async selectRecords(
    ...
    const allFields = new Set<string>(selectFields);
    if (conditions) {
      for (const c of conditions.keys()) allFields.add(c);
    }

    const fieldData = new Map<string, Map<number, string>>();
    ...
    if (conditions && conditions.size > 0) {
      ...
      for (const [condField, condValue] of conditions) {
        ...
        for (const [l, v] of records) {
          if (v === condValue) lines.add(l);
        }
        if (first) { matching = lines; first = false; }
        else {
          const inter = new Set<number>();
          for (const l of matching) if (lines.has(l)) inter.add(l);
          matching = inter;
        }
      }
      ...
      for (const [fn, records] of fieldData) {
        ...
        for (const [l, v] of records) {
          if (matching.has(l)) filtered.set(l, v);
        }
    ...
    for (const r of fieldData.values()) {
      for (const l of r.keys()) allLines.add(l);
    }
    ...
    for (const line of sortedLines) {
      ...
      for (const fn of outputFields) {
        ...
        if (r && r.has(line)) row[fn] = r.get(line) ?? '';
      }
      results.push(row);
    }
    return results;
  ...
  async deleteRecords(
    ...
    let linesToDelete = new Set<number>();
    let first = true;
    ...
    for (const [condField, condValue] of conditions) {
      ...
      if (first) { linesToDelete = lines; first = false; }
      else {
        const inter = new Set<number>();
        for (const l of linesToDelete) if (lines.has(l)) inter.add(l);
        linesToDelete = inter;
      }
    }

    if (linesToDelete.size === 0) return 0;
  ...
  async updateRecords(
    ...
    if (schemas) {
      ...
      if (!validation.valid) return { success: false, count: 0, errors: validation.errors };
      updates = validation.castedData;
    }

    let linesToUpdate = new Set<number>();
    let first = true;
    ...
    for (const [condField, condValue] of conditions) {
      ...
      if (first) { linesToUpdate = lines; first = false; }
      else {
        const inter = new Set<number>();
        for (const l of linesToUpdate) if (lines.has(l)) inter.add(l);
        linesToUpdate = inter;
      }
    }

    if (linesToUpdate.size === 0) return { success: false, count: 0, errors: ['No matching records'] };
  ...
  async clearPoolForDatabase(dbName: string): Promise<void> {
    const prefix = `${dbName}:`;
    const promises: Promise<void>[] = [];
  ...
  async clearPoolForTable(dbName: string, tableName: string): Promise<void> {
